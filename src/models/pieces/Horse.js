...
export class Horse extends ChessPiece {
    ...
    validateMove(newRow, newCol, board) {
        ...
        const legRow =
            Math.abs(rowChange) == 2 ? this.row + rowChange / 2 : this.row;
        const legCol =
            Math.abs(colChange) == 2 ? this.col + colChange / 2 : this.col;

        if (board[legRow][legCol] != null) return false; // leg is blocked
